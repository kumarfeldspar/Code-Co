...
app.get("/discussions/:id", async function (req, res) {
  ...
  await discussion
    ...
    .populate("comments")
    .exec(function (err, founddiscussion) {
      if (err) {
        console.log("something went wrong");
        console.log(err);
      } else {
...
app.get(
  "/discussions/:id/comments/:comment_id/edit",
...
//comment put request
app.put(
  "/discussions/:id/comments/:comment_id",
  checkCommentOwnership,
  async function (req, res) {
...
//comment delet
app.delete(
  "/material/addsuggestions/:suggestion_id",
  checkSuggestionOwnership,
  function (req, res) {
  ...
  if (req.isAuthenticated()) {
    discussion.findById(req.params.id, function (err, founddiscussion) {
      if (err) {
        ...
        if (
          founddiscussion.author.id.equals(req.user._id) ||
    ...
    suggestion.findById(
      ...
      function (err, foundsuggestion) {
        if (err) {
          ...
          //does the user owned comment
          if (
            foundsuggestion.author.id.equals(req.user._id) ||
            req.user.role === "moderator"
          ) {
            next();
          }
